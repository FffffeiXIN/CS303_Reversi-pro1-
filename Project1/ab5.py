...
SOSO = 0
MAX_depth = 2
weight = [[-12000, 260, -50, -40, -40, -50, 260, -12000],
          ...
          [-50, -10, -5, 5, 5, -5, -10, -50],
          [260, 200, -10, -5, -5, -10, 200, 260],
          [-12000, 260, -50, -40, -40, -50, 260, -12000]]
...
def utility(chessboard, player):
    ...
    #         print(diff)
    #     if diff > 0:
    #         return diff * INFINITY, None  # 一个值
    #     elif diff < 0:
    #         return diff * INFINITY, None
    #     else:
    #         return SOSO, None
    ...
    # 开局
        ...
        if (chessboard[0][0] == 1):
            print(chessboard)
            print(str(value) + " " + str(oppo_actions) + " " + str(diff_num) + " " + str(force_corner))
            print(eveal_value)
...
def alphabeta_search(chessboard, player, start_time):
    def max_value(cur_chessboard, cur_depth, alpha, beta):
        ...
        # if time_elapsed_searching > 4.9:
        #     return -INFINITY, None
        cur_candidate_list = find_actions(cur_chessboard, player)
        cur_oppo_list = find_actions(cur_chessboard, -player)
        # 判断游戏是否终止
        if len(cur_candidate_list) == 0 and len(cur_oppo_list) == 0:
            print(cur_chessboard)
            diff = 0
            for i in range(8):
                for j in range(8):
                    if cur_chessboard[i][j] == player:
                        diff = diff - 1
                    elif cur_chessboard[i][j] == -player:
                        diff = diff + 1
            print(diff)
            if diff != 0:
                return diff*1000000, None
            else:
                return SOSO, None
